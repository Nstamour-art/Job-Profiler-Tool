...
def _sanitize_bank_id(bank_id: str) -> str:
    """Return a filesystem-safe filename stem derived from *bank_id*.

    Keeps ASCII letters, digits, hyphens, and underscores; replaces spaces
    with underscores; strips everything else (including path separators,
    Windows drive colons, and dots that could form ``..`` traversal).
    Falls back to ``"default"`` when the sanitised result would be empty.
    """
    safe = re.sub(r"[^\w\s\-]", "", bank_id, flags=re.ASCII)
    safe = re.sub(r"\s+", "_", safe).strip("_")[:100]
