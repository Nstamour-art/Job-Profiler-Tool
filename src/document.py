...
def _bullet(doc: "DocxDocument", text: str, body_pt: int = 10):
...
def _build_resume_sidebar(resume_json: ResumeJSON, personal: dict, education: list,
    ...
    def _main_section(text: str):
        ...
        _set_font(mr, theme.heading_pt, bold=True)
    ...
    for exp in sorted(resume_json.experience, key=_end_year, reverse=True):
        ...
        _set_font(lr, theme.body_pt, bold=True)
        tr = cp.add_run("\t" + exp.dates)
        _set_font(tr, theme.body_pt, bold=True)
        rp = right_cell.add_paragraph()
        _set_space(rp, before_pt=1, after_pt=1)
        rr = rp.add_run(exp.role)
        _set_font(rr, theme.body_pt, italic=True)
    ...
    if resume_json.projects:
        ...
        for proj in resume_json.projects:
            ...
            _set_font(pr, theme.body_pt, bold=True)
...
def build_resume(resume_json: ResumeJSON, personal: dict, education: list,
    ...
    _set_font(contact_run, theme.body_pt)
    ...
    for cat in resume_json.skill_categories:
        ...
        _set_font(label_run, theme.body_pt, bold=True)
        skills_run = p.add_run(", ".join(cat.skills))
        _set_font(skills_run, theme.body_pt)
    ...
    if resume_json.projects:
        ...
        for proj in resume_json.projects:
            ...
            _set_font(title_run, theme.body_pt, bold=True)
            if proj.focus:
                focus_run = p.add_run(f"  —  {proj.focus}")
                _set_font(focus_run, theme.body_pt, italic=True)
            for bullet in proj.bullets:
                _bullet(doc, bullet, body_pt=theme.body_pt)
            if proj.url:
                ...
                _set_font(url_run, theme.body_pt, italic=True)
    ...
    if has_certs:
        ...
        for ed in education:
            ...
            parts = [degree, ed.get("institution", "")]
            text = " | ".join(p for p in parts if p)
            _bullet(doc, text, body_pt=theme.body_pt)
...
def build_cover_letter(cover_json: CoverLetterJSON, personal: dict,
    ...
    for para in cover_json.body_paragraphs:
        _cover_paragraph(doc, para, indent=True, cover_pt=cover_pt)
    ...
    if cover_json.highlights:
        ...
        for item in cover_json.highlights:
            _cover_bullet(doc, item, cover_pt=cover_pt)
