...
from src.memory import MAX_RECALL_ENTRIES, _sanitize_bank_id
...
def test_sanitize_bank_id_strips_path_separators():
    """_sanitize_bank_id removes characters that could enable path traversal."""
    assert _sanitize_bank_id("Jane/Doe") == "JaneDoe"
    assert _sanitize_bank_id("Jane\\Doe") == "JaneDoe"
    assert _sanitize_bank_id("C:/Users/Jane") == "CUsersJane"
    assert _sanitize_bank_id("../../etc/passwd") == "etcpasswd"
    assert _sanitize_bank_id("Jane.Doe") == "JaneDoe"
    assert _sanitize_bank_id("Jane Doe") == "Jane_Doe"
    assert _sanitize_bank_id("") == "default"
    assert _sanitize_bank_id("///") == "default"
